...
def search_trains(request):
    ...
    travel_day = datetime.datetime.strptime(travel_date, "%Y-%m-%d").strftime("%A").capitalize()
    print(f"Filtering trains for {source} to {destination} on {travel_day}")

    # stored_days=list(Train.objects.filter("running_days__name",flat=True).distinct())
    # print(f"stored Running Days in DB: {stored_days}")

    # Get direct trains
    direct_trains = Train.objects.filter(
        source=source,
        destination=destination,
        # **{travel_day.lower(): True}  # Check the boolean field for the weekday
        running_days__name__iexact=travel_day,
    ).distinct()
    print(f" Direct Trains Found: {direct_trains}")

    source_trains = (Train.objects.filter(
        source=source,
        # **{travel_day.lower(): True}
        running_days__name__iexact=travel_day,
    ).distinct())
    ...
    intermediate_source_stations=IntermediateStation.objects.filter(
        station_name=source,
        # train__**{travel_day.lower(): True},
        train__running_days__name__iexact=travel_day,
    ).select_related("train").order_by("arrival_time")

    print(f" Source Trains (Direct Start): {source_trains}")
